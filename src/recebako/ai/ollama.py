...
3. items[].priceは数量を含む税込の行合計(円)。カンマは除去する
4. items[].price_rawには商品行へ実際に印字された行合計をそのまま入れる
5. 商品行や税率別集計に「内」「込」とあればincluded、税が後から加算
   される税抜表示ならexcluded、判断できなければunknownにする
6. 軽減税率記号や税率別集計から読める場合だけitems[].tax_rateを設定する
7. tax_breakdownsにはレシートに明記された税率、対象金額、税額、内税・外税
   区分を入れる。推測した内訳は追加しない
8. 小計、消費税、合計、預り、釣銭などの集計行はitemsへ含めない
9. 値引き行はpriceとprice_rawを負の値にする
10. 全体の読み取りに対する自信をconfidenceに0.0-1.0で正直に申告する
"""
